...
class Application(BaseModel):
    application_id: uuid.UUID = Field(
        None, title="Unique identifier for a given application"
    )
    ...
    callback_url: str = Field(
        None,
        title="Callback URL where access token is returned after authorization",
        max_length=4000,
    )
    public_key: str = Field(
        None,
        title="Public Cryptographic Key used to validate JWT Signatures",
        max_length=4096,
    )
    private_key: str = Field(
        None,
        title="Private Cryptographic Key used to validate JWT Signatures",
        max_length=4096,
    )
    environment: str = Field(
        None,
        title="Tag that describes the application's environment (Ex: Dev, Test, Prod)",
        max_length=50,
    )
    configuration: dict = Field(
        None, title="Additional configurations for the application"
    )
    last_modified: datetime = Field(
        None,
        title="Time stamp of the last modification made to the application definition",
    )
...
class ClaimsProviders(BaseModel):
    ...
    implementation_class: str = Field(
        None, title="Implementation Class", max_length=200
    )
...
class IdentityType(BaseModel):
    ...
    description: str = Field(
        None, title="Description for an Identity Type", max_length=256
    )
...
class Claim(BaseModel):
    ...
    application_id: uuid.UUID = Field(
        None, title="Unique identifier for an application id"
    )
...
class Identity(BaseModel):
    ...
    last_modified: datetime = Field(
        None, title="Timestamp for the last time made to the Identity definition"
    )
...
class ApplicationIdentity(BaseModel):
    identity_id: uuid.UUID = Field(None, title="Unique identifier for an Identity")
    application_id: uuid.UUID = Field(
        None, title="Unique identifier for an Application"
    )
    created: datetime = Field(
        None, title="Timestamp when the Application Identity was created"
    )
...
class ApplicationOwnership(BaseModel):
    ...
    created: datetime = Field(
        None, title="Timestamp when the Application Ownership was created"
    )
    from_date: datetime = Field(
        None, title="Timestamp for when Application Ownership validity starts"
    )
    until_date: datetime = Field(
        None, title="Timestamp for when Application Ownership validity ends"
    )
    is_owner: bool = Field(None, title="If the user is owner")
    is_manager: bool = Field(None, title="If the user is manager")
    configuration: dict = Field(
        None, title="Additional configuration for Application Ownership"
    )
...
class IdentityClaim(BaseModel):
    ...
    identity_id: uuid.UUID = Field(None, title="Unique identifier for an identity")
    from_date: datetime = Field(
        None, title="Timestamp for when Identity Claim validity starts"
    )
    until_date: datetime = Field(
        None, title="Timestamp for when Identity Claim validity ends"
    )
...
class IdentityGroup(BaseModel):
    group_id: uuid.UUID = Field(None, title="Unique identifier for Group")
    application_id: uuid.UUID = Field(
        None, title="Unique identifier for an applications"
    )
    identity_id: uuid.UUID = Field(None, title="Unique identifier for an identity")
